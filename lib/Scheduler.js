...
class Scheduler {
    ...
    load(rootDir, configPath, container) {
        ...
        if (this.configuration.tasks) {
            for (const key of Object.keys(this.configuration.tasks)) {
                ...
                const frequency = task.frequency !== undefined ? task.frequency : 1;
                const singleRun = task.singleRun !== undefined ? task.singleRun : false;

                this.register(key, require(filePath), isSingleton, frequency, singleRun)
    ...
    createTask(name) {
        ...
        if (typeof definition.class === 'function') {
            task = new (Function.prototype.bind.apply(definition.class))(this.container, definition.frequency, definition.singleRun);
        } else {
    ...
    run(timeout=1000, callback=null) {
        return setInterval(() => {
            ...
            for (const taskName in this.getTasks()) {
                if (this.getTasks()[taskName] != null && this.getTasks()[taskName] != undefined){
                    ...
                    if (this.taskTimeouts[taskName] >= this.getTasks()[taskName].frequency) {
                        try {
                            ...
                            if (this.getTasks()[taskName].singleRun === true) {
                                delete this.getTasks()[taskName];
                            }
                        } catch (error) {
                            // Fix me : return specific smart-task exception
                            console.log(error);
                        }
     ...
     * @throws {Error}
     */
    register(name, taskClass, isSingleton, frequency, singleRun) {
        if (this.hasTask(name)) {
            throw new Error(`The task "${name}" already exists`);
        }
        this.taskDefinitions[name] = new TaskDefinition(name, taskClass, isSingleton, frequency, singleRun);
